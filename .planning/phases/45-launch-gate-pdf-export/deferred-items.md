...
- **WIDGET-01 (Department Capacity Gauges empty frame)** — NOT a nested-SVG serialization problem. Root cause: the `domToImageCapture` filter in `src/features/dashboard/pdf-export/svg-snapshot.ts` stripped every `<button>` element wholesale, and each gauge is rendered inside a `<button>` wrapper (`src/components/charts/capacity-gauges.tsx:88`) for navigation. **Fix:** Narrowed the filter so buttons/selects/`role="button"` nodes whose subtree contains `.recharts-wrapper` or `<svg>` are preserved; only buttons without chart descendants are stripped. See commit `1ed32a2`, Vitest `TC-PDF-004`. Confirmed by browser automation smoke test: all 5 gauges with percentages, person counts, and trend indicators visible.

- **WIDGET-02 (Availability Finder shrunken ~20%)** — NOT a `react-window` / virtualization problem. The Availability Finder widget uses a plain `data.results.map(...)` (no `react-window`, no `content-visibility`). Root cause (discovered via live DOM inspection): the widget renders at its full content height (~3107px). `html-to-image` captured a 1536x3107 image; react-pdf's `chartImage` `maxHeight: 350` then scaled it down to a ~174x350px sliver. **Fix (two parts):** (1) `MAX_CAPTURE_HEIGHT = 1200` cap added to `domToImageCapture` in `svg-snapshot.ts` (commit `93ce8bf`) — limits captured height to a manageable 1200px; (2) `maxHeight` in `chartImage` style in `dashboard-pdf-document.tsx` bumped from 350 to 600 (commit `4cb3880`) so the capped image renders at a usable height. An intermediate parent-width fallback (commit `535526f`) was reverted as it was harmful for grid-cell widgets. See Vitest `TC-PDF-005`. Confirmed by browser automation smoke test: filter bar, 19 person count, 7 person rows with hours bars and totals visible.
