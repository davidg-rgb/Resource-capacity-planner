...
[awaiting CI + prod DB access for Tests 1 + 3]
...
result: [pending — requires CI run; diagnostic spec `e2e/_viewport/_diagnostic.spec.ts` in place, uses test.info().attach + console.log, zero assertions per D-04]
