...
- [x] **INPUT-01**: Person Input Form displays AG Grid with months as columns, projects as rows, hours as cell values (F-003)
- [x] **INPUT-02**: User can click a cell, type hours (0-999), and the value saves on blur within 500ms (F-003, F-020)
...
- [x] **INPUT-10**: Keyboard navigation: Tab, Enter, Arrow keys, Escape (F-021)
- [x] **INPUT-11**: Custom clipboard paste handler (AG Grid Community limitation — Research finding) (F-021)
- [x] **INPUT-12**: Past months are read-only, current + future months are editable (A4)
- [x] **INPUT-13**: Auto-save on cell blur with debounced batch upsert (F-020)
