...
- [x] **FOUND-02**: `withTenant()` ORM query wrapper enforcing tenant isolation on every database query (Research: critical risk)
...
- [x] **INPUT-09**: Drag-to-fill custom implementation to replicate value across months (F-021)
...
- [x] **INPUT-14**: Conflict detection — warn if another user modified the same cell (F-020)
...
- [x] **IMPEX-03**: Column mapping with auto-detection of Swedish headers (Namn, Projekt, Timmar, Månad, Avdelning) and English headers (F-007)
- [x] **IMPEX-04**: Validation step shows ready/warning/error counts with actionable suggestions (F-008)
- [x] **IMPEX-05**: Name fuzzy matching — suggest corrections for typos (e.g., "Johan Nilson" → "Johan Nilsson") (F-008)
- [x] **IMPEX-06**: Pivot/grid format detection and automatic unpivoting to flat format (F-026)
- [x] **IMPEX-07**: Import executes in a single database transaction with rollback on failure (F-006)
