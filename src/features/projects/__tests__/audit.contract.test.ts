// v7.0 — Phase 54 / Plan 54-03: AUDIT-02 contract test.
//
// Asserts that POST/PATCH/DELETE on /api/projects/* each emit a change_log row
// via the register.service.ts dispatcher (Plan 54-02 refactored the legacy
// project.service.ts to delegate). Three rows expected for a single
// create → update → delete sequence. DELETE also leaves the row with
// status='archived' AND archivedAt != null (v4 archive semantics preserved).
//
...
const listRoutePromise = import('@/app/api/projects/route');
const idRoutePromise = import('@/app/api/projects/[id]/route');
...
function createPayload(programId: string) {
  return { name: 'Atlas', programId, status: 'active' };
}
...
describe('AUDIT-02: /api/projects/* writes change_log rows via register.service.ts', () => {
  it('POST writes 1 REGISTER_ROW_CREATED row with actor_persona_id=userId', async () => {
    const { programId } = await seedRefs();
    const { POST } = await listRoutePromise;
    const response = await POST(
      req('POST', 'http://test/api/projects', createPayload(programId)) as never,
    );
    ...
    const logs = await testDb
      ...
      .where(
        and(
          eq(schema.changeLog.entity, 'project'),
  ...
  it('PATCH writes 1 REGISTER_ROW_UPDATED row', async () => {
    ...
    const { project: created } = await createResp.json();

    const patchResp = await PATCH(
      req('PATCH', `http://test/api/projects/${created.id}`, updatePayload) as never,
      { params: Promise.resolve({ id: created.id }) },
    );
    expect(patchResp.status).toBe(200);
    const patchBody = await patchResp.json();
    expect(patchBody).toHaveProperty('project');
  ...
  it('DELETE writes 1 REGISTER_ROW_DELETED row, returns 204, and archives the row', async () => {
    const { programId } = await seedRefs();
    const { POST } = await listRoutePromise;
    ...
    const deleteResp = await DELETE(
      req('DELETE', `http://test/api/projects/${created.id}`) as never,
      { params: Promise.resolve({ id: created.id }) },
    );
  ...
  it('create → update → delete writes exactly 3 change_log rows in order', async () => {
    ...
    await PATCH(req('PATCH', `http://test/api/projects/${created.id}`, updatePayload) as never, {
      params: Promise.resolve({ id: created.id }),
    });
    await DELETE(req('DELETE', `http://test/api/projects/${created.id}`) as never, {
      params: Promise.resolve({ id: created.id }),
    });
