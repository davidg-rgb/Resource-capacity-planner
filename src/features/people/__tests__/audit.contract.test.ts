// v7.0 — Phase 54 / Plan 54-03: AUDIT-01 contract test.
//
// Asserts that POST/PATCH/DELETE on /api/people/* each emit a change_log row
// via the register.service.ts dispatcher (Plan 54-02 refactored the legacy
// person.service.ts to delegate). Three rows expected for a single
...
const listRoutePromise = import('@/app/api/people/route');
const idRoutePromise = import('@/app/api/people/[id]/route');
...
async function seedRefs() {
  ...
  const [disc] = await testDb
    ...
    .returning();
  return { departmentId: dept.id, disciplineId: disc.id };
}

function createPayload(departmentId: string, disciplineId: string) {
  return {
    firstName: 'Alice',
    lastName: 'Anderson',
    departmentId,
...
describe('AUDIT-01: /api/people/* writes change_log rows via register.service.ts', () => {
  it('POST writes 1 REGISTER_ROW_CREATED row with actor_persona_id=userId', async () => {
    const { departmentId, disciplineId } = await seedRefs();
    const { POST } = await listRoutePromise;
    const response = await POST(
      req('POST', 'http://test/api/people', createPayload(departmentId, disciplineId)) as never,
    );
    expect(response.status).toBe(201);
    const body = await response.json();
    expect(body).toHaveProperty('person');
    const created = body.person;
    ...
    const logs = await testDb
      ...
      .where(
        and(
          eq(schema.changeLog.entity, 'person'),
  ...
  it('PATCH writes 1 REGISTER_ROW_UPDATED row', async () => {
    ...
    const { person: created } = await createResp.json();

    const patchResp = await PATCH(
      req('PATCH', `http://test/api/people/${created.id}`, updatePayload) as never,
      { params: Promise.resolve({ id: created.id }) },
    );
    expect(patchResp.status).toBe(200);
    const patchBody = await patchResp.json();
    expect(patchBody).toHaveProperty('person');
  ...
  it('DELETE writes 1 REGISTER_ROW_DELETED row and returns 204', async () => {
    ...
    const deleteResp = await DELETE(
      req('DELETE', `http://test/api/people/${created.id}`) as never,
      {
        params: Promise.resolve({ id: created.id }),
      },
    );
  ...
  it('create → update → delete writes exactly 3 change_log rows in order', async () => {
    ...
    await PATCH(req('PATCH', `http://test/api/people/${created.id}`, updatePayload) as never, {
      params: Promise.resolve({ id: created.id }),
    });
    await DELETE(req('DELETE', `http://test/api/people/${created.id}`) as never, {
      params: Promise.resolve({ id: created.id }),
    });
