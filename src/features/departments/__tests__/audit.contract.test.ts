// v7.0 — Phase 54 / Plan 54-03: AUDIT-04 contract test.
//
// Asserts that POST/PATCH/DELETE on /api/departments/* each emit a change_log
// row via the register.service.ts dispatcher (Plan 54-02 refactored the legacy
// department.service.ts to delegate). Three rows expected for a single
...
const listRoutePromise = import('@/app/api/departments/route');
const idRoutePromise = import('@/app/api/departments/[id]/route');
...
const createPayload = { name: 'Engineering' };
const updatePayload = { name: 'Engineering v2' };
...
describe('AUDIT-04: /api/departments/* writes change_log rows via register.service.ts', () => {
  it('POST writes 1 REGISTER_ROW_CREATED row with actor_persona_id=userId', async () => {
    const { POST } = await listRoutePromise;
    const response = await POST(req('POST', 'http://test/api/departments', createPayload) as never);
    expect(response.status).toBe(201);
    const body = await response.json();
    expect(body).toHaveProperty('department');
    const created = body.department;
  ...
  it('PATCH writes 1 REGISTER_ROW_UPDATED row', async () => {
    ...
    const createResp = await POST(
      req('POST', 'http://test/api/departments', createPayload) as never,
    );
    const { department: created } = await createResp.json();

    const patchResp = await PATCH(
      req('PATCH', `http://test/api/departments/${created.id}`, updatePayload) as never,
      { params: Promise.resolve({ id: created.id }) },
    );
    expect(patchResp.status).toBe(200);
    const patchBody = await patchResp.json();
    expect(patchBody).toHaveProperty('department');
  ...
  it('DELETE writes 1 REGISTER_ROW_DELETED row and returns 204', async () => {
    ...
    const deleteResp = await DELETE(
      req('DELETE', `http://test/api/departments/${created.id}`) as never,
      { params: Promise.resolve({ id: created.id }) },
    );
  ...
  it('create → update → delete writes exactly 3 change_log rows in order', async () => {
    ...
    await PATCH(req('PATCH', `http://test/api/departments/${created.id}`, updatePayload) as never, {
      params: Promise.resolve({ id: created.id }),
    });
    await DELETE(req('DELETE', `http://test/api/departments/${created.id}`) as never, {
      params: Promise.resolve({ id: created.id }),
    });
