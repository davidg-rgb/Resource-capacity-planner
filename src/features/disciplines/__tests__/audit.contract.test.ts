// v7.0 — Phase 54 / Plan 54-03: AUDIT-05 contract test.
//
// Asserts that POST/PATCH/DELETE on /api/disciplines/* each emit a change_log
// row via the register.service.ts dispatcher (Plan 54-02 refactored the legacy
// discipline.service.ts to delegate). Three rows expected for a single
...
const listRoutePromise = import('@/app/api/disciplines/route');
const idRoutePromise = import('@/app/api/disciplines/[id]/route');
...
const createPayload = { name: 'Software', abbreviation: 'SW' };
const updatePayload = { name: 'Software v2' };
...
describe('AUDIT-05: /api/disciplines/* writes change_log rows via register.service.ts', () => {
  it('POST writes 1 REGISTER_ROW_CREATED row with actor_persona_id=userId', async () => {
    const { POST } = await listRoutePromise;
    const response = await POST(req('POST', 'http://test/api/disciplines', createPayload) as never);
    expect(response.status).toBe(201);
    const body = await response.json();
    expect(body).toHaveProperty('discipline');
    const created = body.discipline;
    ...
    const logs = await testDb
      ...
      .where(
        and(
          eq(schema.changeLog.entity, 'discipline'),
  ...
  it('PATCH writes 1 REGISTER_ROW_UPDATED row', async () => {
    ...
    const createResp = await POST(
      req('POST', 'http://test/api/disciplines', createPayload) as never,
    );
    const { discipline: created } = await createResp.json();

    const patchResp = await PATCH(
      req('PATCH', `http://test/api/disciplines/${created.id}`, updatePayload) as never,
      { params: Promise.resolve({ id: created.id }) },
    );
    expect(patchResp.status).toBe(200);
    const patchBody = await patchResp.json();
    expect(patchBody).toHaveProperty('discipline');
  ...
  it('DELETE writes 1 REGISTER_ROW_DELETED row and returns 204', async () => {
    ...
    const deleteResp = await DELETE(
      req('DELETE', `http://test/api/disciplines/${created.id}`) as never,
      { params: Promise.resolve({ id: created.id }) },
    );
  ...
  it('create → update → delete writes exactly 3 change_log rows in order', async () => {
    ...
    await PATCH(req('PATCH', `http://test/api/disciplines/${created.id}`, updatePayload) as never, {
      params: Promise.resolve({ id: created.id }),
    });
    await DELETE(req('DELETE', `http://test/api/disciplines/${created.id}`) as never, {
      params: Promise.resolve({ id: created.id }),
    });
