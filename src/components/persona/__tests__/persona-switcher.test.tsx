  ...
  describe('legacy mode (uiV6Landing OFF)', () => {
      ...
      fetchMock.mockImplementation(async (url: string) => {
        if (typeof url === 'string' && url.startsWith('/api/people')) {
          return {
            ok: true,
            status: 200,
            json: async () => ({ people: [{ id: 'p1', firstName: 'A', lastName: 'B' }] }),
          };
        }
    ...
    describe('department sub-picker (UNBREAK-01/02/08)', () => {
        ...
        fetchMock.mockImplementation(async (url: string) => {
          ...
          if (typeof url === 'string' && url.startsWith('/api/departments')) {
            return {
              ok: true,
              status: 200,
              json: async () => ({ departments: [{ id: 'a', name: 'Konstruktion' }] }),
            };
          }
          return { ok: false, status: 404, json: async () => ({}) };
        });
      ...
      it('shows no-department hint when 0 departments', async () => {
        ...
        await waitFor(() => {
          const kindSelect = screen.getAllByRole('combobox')[0];
          const lmOption = kindSelect.querySelector(
      ...
      fetchMock.mockImplementation(async (url: string) => {
        if (typeof url === 'string' && url.startsWith('/api/people')) {
          return {
            ok: true,
            status: 200,
            json: async () => ({ people: [{ id: 'p1', firstName: 'Anna', lastName: 'PM' }] }),
          };
        }
        if (typeof url === 'string' && url.startsWith('/api/departments')) {
          return {
            ok: true,
            status: 200,
            json: async () => ({ departments: [{ id: 'd1', name: 'Konstruktion' }] }),
          };
        }
        return { ok: false, status: 404, json: async () => ({}) };
      });
      ...
      fetchMock.mockImplementation(async (url: string) => {
        if (typeof url === 'string' && url.startsWith('/api/people')) {
          return {
            ok: true,
            status: 200,
            json: async () => ({ people: [{ id: 'p-solo', firstName: 'Solo', lastName: 'PM' }] }),
          };
        }
        if (typeof url === 'string' && url.startsWith('/api/departments')) {
          return {
      ...
      fetchMock.mockImplementation(async (url: string) => {
        ...
        if (typeof url === 'string' && url.startsWith('/api/departments')) {
          return {
            ok: true,
            status: 200,
            json: async () => ({ departments: [{ id: 'd-solo', name: 'Konstruktion' }] }),
          };
        }
        return { ok: false, status: 404, json: async () => ({}) };
      });
