...
import { DISCIPLINE_SUGGESTIONS } from '@/features/onboarding/onboarding.constants';
import type { Discipline } from './onboarding-wizard';

interface StepDisciplinesProps {
  existingDisciplines: Discipline[];
  onAdd: (disc: Discipline) => void;
  onNext: () => void;
  onSkipAll: () => void;
}
...
export function StepDisciplines({
  ...
  const existingNames = new Set(existingDisciplines.map((d) => d.name.toLowerCase()));

  async function createDiscipline(name: string, abbreviation: string) {
    if (adding) return;
    setInlineError(null);
    setAdding(name);
    ...
    try {
      ...
      onAdd(data.discipline);
    } catch {
      toast.error(`Failed to create discipline "${name}"`);
    } finally {
      setAdding(null);
    }
  }
  ...
  async function handleAddCustom() {
    ...
    const trimmedAbbr = customAbbr.trim();
    if (!trimmedName || !trimmedAbbr) return;
    if (existingNames.has(trimmedName.toLowerCase())) {
      setInlineError('Already exists');
      return;
    }
    await createDiscipline(trimmedName, trimmedAbbr);
    setCustomName('');
    setCustomAbbr('');
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        {DISCIPLINE_SUGGESTIONS.map((disc) => {
          const isExisting = existingNames.has(disc.name.toLowerCase());
          const isAdding = adding === disc.name;
          ...
          return (
            <button
              ...
              onClick={() => createDiscipline(disc.name, disc.abbreviation)}
            ...
            >
              {isExisting
                ? `${disc.name} (${disc.abbreviation}) (added)`
                : `${disc.name} (${disc.abbreviation})`}
            </button>
          );
        })}
      </div>
      ...
      <div className="flex gap-2">
        ...
        <input
          ...
          onKeyDown={(e) => e.key === 'Enter' && handleAddCustom()}
          placeholder="Abbr"
          className="bg-surface text-on-surface placeholder:text-on-surface-variant border-outline-variant focus:border-primary w-20 rounded border px-3 py-2 text-sm outline-none"
        />
        <button
          type="button"
          onClick={handleAddCustom}
          disabled={!customName.trim() || !customAbbr.trim() || !!adding}
