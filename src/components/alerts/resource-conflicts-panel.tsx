...
// v6.0 — Phase 53 / Plan 53-05 (POLISH-05): route-agnostic panel extracted
// from `src/features/dashboard/widgets/resource-conflict-widget.tsx`. The
// widget body (ResourceConflictContent) moved here verbatim so the same
// implementation can render both inside the dashboard widget shell AND on
// the new `/alerts?tab=conflicts` surface.
//
// Behavior preserved from the legacy widget:
//   - redistribute modal (batch allocation save)
//   - dismissed-conflicts persisted in localStorage under
//     `nordic-capacity-dismissed-conflicts`
//   - 3 mutations (apply suggestion, redistribute save, dismiss)
//   - viewMode toggle (current month vs next 3 months)
//   - sort by severity; show-more threshold at 3 cards
//
// The panel accepts an optional `timeRange` prop so callers outside the
// widget shell (AlertsPage) can omit it and get a sensible default
// (current month + 3 months). The widget still passes `timeRange` through
// exactly as before so nothing changes on the dashboard.
...
export interface ResourceConflictsPanelProps {
  /** Optional time range; defaults to current month + 3-month window. */
  timeRange?: { from: string; to: string };
}

/**
 * Route-agnostic resource conflicts panel.
 *
 * Extracted from the legacy `resource-conflict-widget.tsx` widget body so
 * the same implementation can render inside the dashboard widget shell AND
 * on the new `/alerts?tab=conflicts` surface (Plan 53-05 POLISH-05).
 *
 * Accepts an optional `timeRange` prop. When absent (e.g. called from
 * AlertsPage without dashboard context), defaults to a window starting at
 * the current month and spanning 4 months, matching the TopNav alert badge.
 */
export const ResourceConflictsPanel = React.memo(function ResourceConflictsPanel({
  timeRange,
}: ResourceConflictsPanelProps) {
  const t = useTranslations('widgets.resourceConflicts');
  const tc = useTranslations('widgets.common');
  ...
  return (
    <div className="space-y-4" data-testid="resource-conflicts-panel">
      {/* Controls */}
...
// ---------------------------------------------------------------------------
// Default time-range helper for consumers outside the widget shell
// ---------------------------------------------------------------------------

/**
 * Convenience helper: returns the same 4-month window the TopNav alert
 * badge and StrategicAlertsBanner use. Pass the result as `timeRange` to
 * the panel when mounting outside a dashboard grid.
 */
export function defaultConflictsTimeRange(): { from: string; to: string } {
  const from = getCurrentMonth();
  const to = generateMonthRange(from, 4).at(-1)!;
  return { from, to };
}
